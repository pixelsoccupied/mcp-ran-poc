...
spec = importlib.util.spec_from_file_location(
    "root_agent", parent_dir / "adk-agents" / "root_agent.py"
)
        ...
        try:
            ...
            if not session_id_to_use:
                new_session = await runner.session_service.create_session(
                    app_name=APP_NAME, user_id=request.user_id, state={}
                )
            ...
            for event in events:
                ...
                if event.is_final_response():
                    ...
                    hasattr(event, "content") and event.content and event.content.parts
                ):
            ...
            error_response = {
                ...
                "error": str(e),
                "session_id": session_id_to_use
                if "session_id_to_use" in locals()
                else None,
            }
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        events = runner.run(
            user_id=request.user_id, session_id=session_id_to_use, new_message=content
        )
            ...
            if hasattr(event, "get_function_responses"):
                try:
                    ...
                    if function_responses:
                        logger.info(
                            f"Function responses found: {len(function_responses)}"
                        )
                        for response in function_responses:
                            logger.info(
                                f"Tool response: {getattr(response, 'name', 'unknown')}"
                            )
                except Exception as e:
...
async def get_agent_info():
    ...
    return {
        ...
        "instruction": root_agent.instruction[:200] + "..."
        if len(root_agent.instruction) > 200
        else root_agent.instruction,
