    ...
    try:
        ...
        if response.status_code == 200:
                ...
                if line.startswith("data: "):
                    try:
                        ...
                        if data["type"] == "tool_step":
                            ...
                            tool_content = "**Real-time Tool Execution:**\n" + "\n".join(tool_steps)
                            tool_response = gr.ChatMessage(
                                content=tool_content,
                                metadata={"title": f"🛠️ Step {current_step}", "status": "pending"}
                            )
                            ...
                            if tool_steps:
                                final_tool_content = "**Tool Execution Complete:**\n" + "\n".join(tool_steps)
                                final_tool_response = gr.ChatMessage(
                                    content=final_tool_content,
                                    metadata={"title": "✅ Tools Complete", "status": "done"}
                                )
                                final_messages = [
                                    thinking_response,
                                    final_tool_response,
                            ...
                            error_msg = gr.ChatMessage(content=f"❌ Error: {data['error']}")
            ...
            if not agent_response:
                error_msg = gr.ChatMessage(content="❌ No response received from backend")
                yield error_msg, history, session_id
        ...
        if response.status_code == 200:
            ...
            if tools_count > 0:
                ...
                tool_content = f"**Tool Execution Steps:**\n" + "\n".join(tool_steps)
            ...
            error_msg = gr.ChatMessage(content=f"❌ Error: {response.status_code} - {response.text}")
            yield error_msg, history, session_id
            
    except requests.exceptions.ConnectionError:
        error_msg = gr.ChatMessage(content="❌ Cannot connect to backend. Make sure it's running on port 8000.")
        yield error_msg, history, session_id
...
with gr.Blocks(title="RAN Assistant") as demo:
    ...
    gr.Markdown("Chat with your ADK agent for database and cluster operations. Session memory maintained across conversations.")
